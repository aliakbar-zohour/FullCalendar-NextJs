    ...
    <div className="p-4">
      ...
      <FullCalendar
        ...
        eventContent={(arg) => {
          ...
          if (isBlocked) {
            return (
              <div className="w-full h-full bg-gray-300 opacity-70 rounded-md" />
            );
          }
      ...
      <AnimatePresence>
        {isCreateOpen && (
          ...
          >
            <motion.div
              className="bg-white rounded-2xl shadow-lg w-full h-full max-w-md mx-auto p-6 flex flex-col"
              initial={{ y: "100%" }}
              animate={{ y: 0 }}
              exit={{ y: "100%" }}
            ...
            >
              ...
              <DatePicker
                value={newStart}
                onChange={(date) =>
                  setNewStart(date?.toDate?.().toISOString() || "")
                }
              ...
              <DatePicker
                value={newEnd}
                onChange={(date) =>
                  setNewEnd(date?.toDate?.().toISOString() || "")
                }
                format="YYYY/MM/DD HH:mm"
                calendar={persian}
                locale={persian_fa}
      ...
      {/* مودال جزئیات (همون قبلی با ویرایش/حذف) */}
      <AnimatePresence>
        {selectedEvent && (
          <motion.div
            className="fixed inset-0 bg-black/70 flex items-center justify-center z-50"
          ...
          >
            <motion.div
              className="bg-white rounded-2xl shadow-lg w-full h-full max-w-2xl mx-auto p-6 flex flex-col"
              initial={{ y: "100%" }}
              animate={{ y: 0 }}
              exit={{ y: "100%" }}
              transition={{ type: "spring", stiffness: 120, damping: 20 }}
